...
Then define the workspace paths for the current shell:
...
scripts/generate_observations.sh \
  ...
  --engines mcts,minimax,beam \
  --mcts-iterations 1500 \
  --minimax-depth 6 \
  --beam-width 64 \
  --seeds 10 \
  --workers 4
...
```bash
cd "$MODELS"
PYTHONPATH="$CORE_PY/src:$MODELS/src" python -m quantik_models.data.materialize \
