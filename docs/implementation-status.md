...
| `opening-book-summary.v1` | Producer emits summary JSON with release `1.2.0` | Producer emits summary JSON with release `1.2.0` | Cross-stack summary artifact is implemented and validated by contracts scripts. |
| `observation.v1` | JSONL reader plus optional PyArrow Parquet reader/writer validate release `1.2.0`, metadata, physical schema, bitboards, legal mask, policy visits, and scalar fields | Benchmark exporter emits release `1.2.0`; consumer parser plus optional Arrow/Parquet reader/writer validate release, metadata, physical schema, bitboards, legal mask, policy visits, and scalar fields | Rust/Python Parquet parity is implemented for the current required physical column surface. |
| `game-result.v1` | JSONL reader plus optional PyArrow Parquet reader/writer validate release `1.2.0`, metadata, physical schema, winner, plies, move indices, and required engine fields | Benchmark exporter emits release `1.2.0`; consumer parser plus optional Arrow/Parquet reader/writer validate release, metadata, physical schema, winner, plies, move indices, and required engine fields | Rust/Python Parquet parity is implemented for the current required physical column surface. |
| `model-checkpoint.v1` | Manifest loader/parser validates release `1.2.0`, fields, supported inputs, weights format, and fixture; `quantik-models-py` produces checkpoints (`weights.safetensors`, training report, manifest) via `quantik-models-train`, validated through the loader | Manifest parser validates release `1.2.0`, fields, supported inputs, weights format, and fixture | Rust/Python manifest parity is implemented, and the contract now has a produced surface: the model repo's trainer exports safetensors checkpoints whose manifests round-trip through the Python loader. |
