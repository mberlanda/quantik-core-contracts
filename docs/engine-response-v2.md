...
Status: **registered** — schema `schemas/engine-response-v2.json`
(JSON Schema 2020-12), fixtures `fixtures/engine-response/engine-response-v2-*.jsonl`,
rejection cases `fixtures/engine-response/engine-response-v2-invalid.json`.

`engine-response.v2` is the extended response body an engine service returns for
[`engine-request.v1`](engine-request-v1.md) (requests are not extended). It adds
ranked `candidates`, a principal variation `pv`, an optional `engine_config`, and a
**required** `certainty`. [`engine-response.v1`](engine-response-v1.md) is frozen and
keeps validating; a v2 body never omits `certainty`.

This file is the QW-018 W1 design record (DECIDED 2026-09-20, all five
recommendations accepted as written), moved here from `engine-response-v1.md` by
QW-018 W2, which registered the schema. No server emits v2 yet (W3 and W4).

## Fields

```text
schema           "engine-response.v2"     required
...
certainty        "estimate" | "proof"     required
value            number in [-1, 1]        optional, mover-relative
policy           64 numbers               optional, unchanged from v1
candidates       1..=64 entries           optional, best first
                 {action_index 0..=63, score number, unit visits|logit|prior|value}
pv               1..=64 action indices    optional, pv[0] == action_index
engine_config    non-empty string         optional
```

The top level and each candidate are closed (`additionalProperties: false`).

What the schema checks per unit: `visits` is a non-negative integer, `prior` is in
`[0, 1]`, `value` is in `[-1, 1]`, `logit` is any number.

What `scripts/validate_contracts.py` checks that JSON Schema cannot: `pv[0]`
equals `action_index`, and candidate `action_index` values are unique within a
response. Not enforced by either, and left to producers: candidates are legal
moves only, are ranked best first, and `certainty: proof` is never sent on the
strength of a heuristic, network output or sampled search (section 3). Mixed
units in one list are not rejected (section 1: reserved, not forbidden); W2 chose
not to enforce uniformity because a per-entry unit is exactly what makes a
mixed list self-describing.

Fixtures are synthetic, hand-built to cover each case (visits, logit, prior,
value/proof, estimate, PV, minimal); unlike the v1 captures they were not
recorded from a running server.
